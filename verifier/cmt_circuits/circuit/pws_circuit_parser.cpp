...
parsePoly(Tokenizer& pws, const string end)
{
  ...
  while (pws.hasNext())
  {
    ...
    {
      pws.rewind();
      Maybe<GatePosition> term = parsePolyTerm(pws, isMinus);
      if (term.isValid())
  ...
  if (isMinus) {
      ...
      return Maybe<GatePosition>(reduce(GateDescription::SUB, terms));
  }
  else {
...
Maybe<GatePosition> PWSCircuitParser::
parsePolyTerm(Tokenizer& pws, bool& isMinus)
{
  ...
  while (pws.hasNext()) {
      ...
      if (token == "(") {
          ...
          if (vars.empty()) {
              parseError("Unexpected 'minus' in polynomial definition.");
              abort = true;
          }
          else {
              pws.rewind();
