...
constructCircuit()
{
  ...
  int layerIdx = 0;
  for (LayerIt lit = desc.begin(); lit != desc.end(); ++lit, ++layerIdx)
  {
    ...
    // only try to copy in the mux gates if they exist!
    if (layerIdx < parser.muxGates.size()) {
        clayer().muxGates = parser.muxGates[layerIdx];
    }
